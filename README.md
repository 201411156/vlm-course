...
본문을 읽다 모르는 낱말이 나오면 바로 뜻을 볼 수 있습니다.

- `assets/js/glossary.js` 의 항목을 site.js 가 읽어, 각 장 본문에서 **용어의 첫 등장 한 곳만**
  `<abbr class="gl">` 로 감싸고 hover·focus·탭에 팝오버를 띄웁니다.
- 제목·코드·링크·LAB/퀴즈 카드·참고문헌 안은 건드리지 않습니다.
- 장 끝에는 그 장에서 주석된 용어만 모은 "이 장의 용어" 상자가 자동으로 붙습니다.

용어를 추가하려면 `glossary.js` 에 항목 하나를 넣기만 하면 됩니다.

```js
{ id:'고유-슬러그', t:['대표 표기','다른 표기'], k:'분류', c:'첫 등장 장',
  s:'정의 한두 문장.', b:'비전 엔지니어에게 익숙한 비유 (선택).' }
```

`t` 에는 **본문에 실제로 나오는 표기만** 넣습니다. "배치", "위치", "헤드" 처럼 뜻이 갈리는
일반 명사는 오탐이 나므로 반드시 복합어("배치 크기", "위치 인코딩")로만 등록합니다.
