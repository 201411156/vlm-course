/* ==========================================================================
   ...
   전부 브라우저 안에서 도는 장난감 시뮬레이션이다. 실제 모델을 부르지 않는다.
  ...
  function refresh() {
    ...
    if (el.verdict) {
      ...
      if (S.step === 0 && !(mode === 'none' && !unfreeze)) {
        v = '아직 학습 전 — 시각 특징을 그대로 얹은 상태다. ▶ 를 누르세요.';
      } else if (mode === 'none' && !unfreeze) {
        v = '다리도 없고 시각 경로도 얼어 있다 — 학습할 파라미터가 하나도 없다.';
      } else if (mode === 'none' && unfreeze) {
        v = '다리 없이 인코더만 움직여도 정렬된다 — early fusion이 노리는 지점.';
      } else if (S.acc === N && S.loss < 0.35) {
        v = '정렬 완료 — 대각선만 밝다. 이게 CLIP이 만든 그림이다.';
      } else if (S.step > 200 && S.acc < N) {
        v = '수렴했는데도 남는 오차 = 이 프로젝터의 표현력 한계.';
      } else {
        v = '학습 중 — 짝은 당기고, 짝이 아닌 것은 밀어낸다.';
      }
